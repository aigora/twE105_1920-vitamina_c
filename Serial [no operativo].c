...
void Serial_read (Serial *link, char *buffer);
//Función que además de leer los datos que entran por el puerto serie, los preparará para su uso por parte del resto del programa [Todavía no creada]
//void Serial_read_prepare (Serial *link, char *buffer);
...
main (){ //Hay que convertirlo en una función, que puede incluirse en esta parte del programa o en otra
	...
	char buffer_entrada [TAM_BUFFER]; //Buffer en el que la función de lectura guardará los datos recibidos después de leerlos uno a uno.
    ...
    printf ("Intentando recibir mensajes...\n");
    //while (0==0)
    Serial_read (&puertoserie, buffer_entrada);
	printf ("\n\n\n\n_________\nProceso terminado.\n<Fin del programa>");
	return 0;
}
...
void Serial_read (Serial *link, char buffer []){
	printf ("Entrando en la funcion de lectura...\n");
	char x;
	int posicion;
	DWORD bytesread;
	/*//En esta parte se produce un error en la apertura del fichero
	FILE *fichero;
	//Abro el fichero del puerto serie en modo de lectura
	printf ("Intentando abrir el fichero del puerto...\n");
	fichero = fopen (link->handler, "r");
	if (fichero==NULL){
		printf ("Se ha producido un error en la lectura del fichero");
		exit (-1);
	}
	printf ("Fichero abierto");*/
	//Intentando leer el puerto sin abrir el fichero
	
	//Por ahora a modo de prueba voy a leer un único carácter
	printf ("Intentando leer directamente un carcater del fichero\n");
	//fscanf (link->handler, "%c", &x);
	int i=0;
	//while (i<10){
		//i++;
		printf ("Intentando leer\n");
		ClearCommError (link->handler, &link->errors, &link->status);
		printf ("ClearCommErrors ejecutado. Errores: %i. Status: %i\n", link->errors, link->status);
		memset (buffer, 0, TAM_BUFFER);
	//	while (link->status.cbInQue==0){
	//		printf ("Esperando mensaje...\n");
	//	}
	posicion = link->status.cbInQue;
	printf ("Carcateres a leer: %i\n", posicion);
		ReadFile (link->handler, buffer, posicion, &bytesread, NULL);
		//*(buffer+i) = x;
	//}
	printf ("Se ha leido un caracter: %s\n", buffer);

	//Necesito saber el número de carácteres que tengo que leer.
	
	
	//Cierro el fichero (si procede)
	//fclose (fichero);
	printf ("Funcion de lectura finalizada");
}
